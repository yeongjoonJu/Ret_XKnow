...
from retrievers.colbert.utils.utils import torch_load_dnn

from transformers import AutoTokenizer, AutoConfig
from retrievers.colbert.modeling.hf_colbert import class_factory
from retrievers.colbert.infra.config import ColBERTConfig
...
class BaseColBERT(torch.nn.Module):
    ...
    def __init__(self, name_or_path, colbert_config=None):
        ...
        if not "colbert" in name_or_path:
            config = AutoConfig.from_pretrained(name_or_path)
            self.model = HF_ColBERT(config, colbert_config=self.colbert_config)
        else:
